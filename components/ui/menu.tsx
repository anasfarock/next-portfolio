...
function MenuTrigger({ className, ...props }: MenuPrimitive.Trigger.Props) {
  return <MenuPrimitive.Trigger data-slot="menu-trigger" className={cn(
    "rounded-sm p-0.5 text-foreground hover:text-popover-foreground",
    "data-pressed:bg-accent/30 dark:data-pressed:bg-input/50 data-pressed:text-accent-foreground",
    className,
  )} {...props} />;
}
  ...
  return (
    <MenuPrimitive.Portal>
      ...
      >
        <MenuPrimitive.Popup
          className={cn(
            "relative flex not-[class*='w-']:min-w-32 origin-(--transform-origin) rounded-lg border bg-secondary dark:bg-muted/30 not-dark:bg-clip-padding shadow-lg/5 outline-none transition-[transform,opacity] duration-200 ease-out before:pointer-events-none before:absolute before:inset-0 before:rounded-[calc(var(--radius-lg)-1px)] before:shadow-[0_1px_--theme(--color-black/4%)] focus:outline-none dark:before:shadow-[0_-1px_--theme(--color-white/6%)]",
            "data-ending-style:scale-95 duration-200 transition-all data-starting-style:scale-95 data-ending-style:opacity-0 data-starting-style:opacity-0 data-instant:duration-0",
            className,
          )}
...
function MenuItem({
  ...
  return (
    <MenuPrimitive.Item
      className={cn(
        "[&>svg]:-mx-0.5 cursor-pointer flex min-h-8 select-none items-center gap-2 rounded-sm px-2 py-1 text-base text-foreground outline-none data-disabled:pointer-events-none data-highlighted:bg-accent data-inset:ps-8 data-[variant=destructive]:text-destructive-foreground hover:data-[variant=destructive]:bg-destructive/10 data-highlighted:text-accent-foreground data-disabled:opacity-64 sm:min-h-7 sm:text-sm [&>svg:not([class*='opacity-'])]:opacity-80 [&>svg:not([class*='size-'])]:size-4.5 sm:[&>svg:not([class*='size-'])]:size-4 [&>svg]:pointer-events-none [&>svg]:shrink-0",
        className,
      )}
      data-inset={inset}
      data-slot="menu-item"
